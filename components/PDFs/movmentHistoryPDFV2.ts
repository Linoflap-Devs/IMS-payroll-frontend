 ...
 */
export function generateMovementHistoryV2(
    ...
    try {
        ...
        const drawPageHeader = (vesselName: string, crewNumber?: number) => {
            ...
            doc.setFontSize(8);
            doc.setFont('helvetica', 'bold');
            doc.text(mode === 'vessel' ? vesselName : 'ALL VESSELS', margins.left + 2, vesselInfoY + 7.5);
        ...
        const vesselName = mode === 'all' ? 'ALL VESSELS' : crewData[0]?.Movements[0]?.VesselName || 'UNKNOWN VESSEL';
        const totalCrews = crewData.length;
        drawPageHeader(vesselName, totalCrews);
        drawTableHeader();

        // Process all crews and their movements
        crewData.forEach((crew, crewIndex) => {
            crew.Movements.forEach((movement, movementIndex) => {
                ...
                // Draw horizontal line at the top of row (except for the very first row after header)
                ...
                columnKeys.forEach((key, i) => {
                    ...
                    if (key === 'CrewName') {
                        ...
                        value = movement[key as keyof Movement] as string || '';
                    }
        ...
        `VesselMovement_${capitalizeFirstLetter(vesselName || 'UnknownVessel')}_${capitalizeFirstLetter(getMonthName(month))}-${year}.pdf` : 
...
export function generateMovementHistoryPDFV2(
    crewData: CrewMovementHistory[],
    month: number,
    year: number,
    dateGenerated: Date,
    ...
    generateMovementHistoryV2(crewData, month, year, dateGenerated, mode);
}